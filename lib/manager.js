	...
	init(callback) {
		...
		this.rpc.on("UX.ctl.module.create", (req, callback) => {
	   		if(!this.isValid(req.module))
				return callback({error: "Invalid module "+req.module});
		...
		if(path.extname(moduleName) == ".html" && fs.existsSync(moduleName)){
			if(fs.existsSync(path.join(process.cwd(), moduleName))){
		   	...
		   	if(!fs.existsSync(moduleFile)){
		   		moduleFile = path.join(this.app.config.moduleFolder, mFile);
		   		if(!fs.existsSync(moduleFile)) {
	   	...
	   	if(this.app.config) {
	   		if(this.app.config.nats)
	   			µArgs.nats = this.app.config.nats.port;
	   		if(this.app.config.stan)
	   			µArgs.stan = this.app.config.stan.port;
	   	}
	   	...
	   	//console.log("mf:manager: moduleFile, options", moduleFile, options)
