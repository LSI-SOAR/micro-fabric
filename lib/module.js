...
class Module extends EventEmitter {
    constructor(config) {
        ...
        try {
            ...
            console.log("µFabric::Module - no nw window, running headless");
            //console.log(e)
        }
...
*/
        ...
        this.config.verbose && console.log("µFabric module config:", this.config);

        if(this.config.ctorInit !== false)
            dpc(() => { 
                this.config.verbose && console.log("µFabric running module init...");
                this.initModule_(); 
            });
    ...
    deref_() {
        ...
        if(!this.deps) {
            ...
            doInit(() => {
                this.config.verbose && console.log("µFabric - running Module::main()")
                this.main(this.args);
    ...
    main(){
        console.log("µFabric Main: Override me in child class")
    }

    init(fn) {
        this.config.verbose && console.trace("Module:init...");
        if(!fn)
    ...
    initWindowStateEvents(params){
        this.config.verbose && console.log("initWindowStateEvents", this.args)
    ...
    initSTAN() {
        ...
        this.config.verbose && console.log("initSTAN - ",this.config.stan.cluster, this.config.ident || this.config.stan.ident || (this.ident+'-'+NUID.next().toLowerCase()), this.config.stan);
        // this.config.verbose && console.log("STAN config is:", this.config.stan);
...
*/
    ...
    BufferSuffixFromJSON(buffer, suffix) {
