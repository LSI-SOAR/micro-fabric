	...
	initNATS() {
		...
		this.rpc = {
			...
			dispatch : (subject, msg, callback) => {
				if(callback)
					console.warn(`Warning: Please remove callback or use "let {err, resp} = await rpc.request("${subject}", {})"`.yellow);
					...
					this.nats.request(subject, msg, {'max':1}, (response) => {
						try{
							...
							if(response.code && response.code === NATS.REQ_TIMEOUT)
								return done({error: 'NATS RPC request timed out.', code: response.code});
			...
			on : (subject, fn) => {
				...
				return this.rpc.subscribe(subject, async (msg, replyTo, ident) => {
