...
class Module extends EventEmitter {
	...
	deref_() {
		...
		if(!this.deps) {
			...
			doInit(() => {
				console.log("µFabric - running Module::main()")
				this.main(this.args);
	...
	openPopup(module, options, args, callback){
		...
		if(this.popups[module]){
			var popup = this.popups[module];
			popup.show((err)=>{
				if(err)
					return callback(err);
		...
		this.rpc.dispatch("UX.ctl.module.create", { 
			...
			popup.on("closed", ()=>{
				delete this.popups[module];
			});

			this.popups[module] = popup;
	...
	*/
	...
	openPopup(module, options, args, callback){
		this.popups = this.popups || {};
		var key = module+args.popup.ident;
		if(this.popups[key]){
