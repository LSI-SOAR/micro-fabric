					...
					this.nats.request(subject, msg, {'max':1}, (response) => {
						try{
							if(response.error)
								return callback(response.error);
							if(response.code && response.code === NATS.REQ_TIMEOUT)
								return callback({error: 'NATS RPC request timed out.', code: response.code});
							callback(null, response.data);
						}catch(e){
							console.log("(NATS RPC) Error in user callback - subject:",subject,"msg:",msg,"error:", e, e.stack);
						}
					})
			},
			on : (subject, fn) => {
				let pattern = /[\*\>]/.test(subject);
				return this.nats.subscribe(subject, (msg, replyTo, ident) => {
					...
					try{
						...
						console.log("(NATS *) Error in user callback - subject:",subject,"ident:",ident,"error:", e, e.stack,"fn:",fn);
					}
				})
			},
			off : (...args) => {
