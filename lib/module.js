...
class Module extends EventEmitter {
    constructor(config) {
            ...
            if(_args && _args.length) {
                ...
                if(this.moduleArgs.width != undefined && this.moduleArgs.height != undefined){
                    try{
                        ...
                        win.resizeTo(this.moduleArgs.width, this.moduleArgs.height);
                    }catch(e){
                ...
                if(this.moduleArgs.x != undefined && this.moduleArgs.y != undefined){
                    try{
                        let win = nw.Window.get();
                        win.moveTo(this.moduleArgs.x, this.moduleArgs.y);
                    }catch(e){
        ...
        console.log("Module args:", args, this.moduleArgs);

        this.uid = config.uid || (args ? args.uid : null) || '@';
        this.args = args;
        ...
        console.log("module config:".cyan.bold, this.config);

        if(this.config.ctorInit !== false)
            dpc(() => { 
                console.log("Running module init...".magenta.bold);
                this.initModule_(); 
            });
    ...
    initModule_() {
        console.log("µFabric::init()", this.config);
    ...
    deref_() {
        console.log("+++ DEREF:".cyan.bold,this.deps);
        this.deps--;
        if(!this.deps) {
            ...
            doInit(() => {
                console.log("RUNNING MAIN".cyan.bold)
                this.main();
            })
    ...
    init(fn) {
        console.trace("Module:init...".blue.bold);
        if(!fn)
    ...
    initNATS() {
		...
		this.nats.on('reconnecting', function() {
			console.log('nats reconnecting'.cyan.bold);
    ...
    initSTAN() {
    	...
    	console.log("connecting STAN to:".yellow.bold,this.config.stan.url.bold+' cluster:'+this.config.stan.cluster+'...');
        ...
        console.log("initSTAN - ",this.config.stan.cluster, this.config.ident || this.config.stan.ident || (this.ident+'-'+NUID.next().toLowerCase()), this.config.stan);
         console.log("STAN config is:".cyan.bold, this.config.stan);
        ...
        this.stan.on('reconnecting', () => {
            ...
            console.log("stan-reconnecting...".cyan.bold);
        })
