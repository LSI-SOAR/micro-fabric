...
class I18nEditor extends EventEmitter{
	...
	init() {
		return new Promise((resolve,reject)=>{
			...
			this.restoreEntries(path.join(this.config.dataFolder, this.config.entriesFile)).then(()=>{
				this.initHash();
				resolve(this.getT());
			});
		})
	...
	getEditorUrl(){
		if(this.config.moduleFile)
			return this.config.moduleFile;
		if(this.config.polymerServe)
			return 'http://127.0.0.1:8081/modules/i18n/i18n-editor.html';

		return '/@BUILD/modules/i18n/i18n-editor.html';
	}

	get logoPath(){
		if(this.config.polymerServe)
			return "./logo.png";
		return "/@BUILD/modules/i18n/logo.png";
	}

	initEvents(){
		var rpc = this.app.rpc;
		rpc.on("fabric-i18n-entries", this.getEntries.bind(this));
		rpc.on("fabric-i18n-get-settings", this.getSettings.bind(this));
		rpc.on("fabric-i18n-create-entry", this.createEntryRequest.bind(this));
	}

	getSettings(args, callback){
		callback(null, {
			locale: this.locale,
			source: this.source,
	...
	setLocale(locale) {
		...
		for(let [uuid, node] of this.nodes)
			node.onLocaleChanged && node.onLocaleChanged(locale);
	}
    ...
    update(args) {
        ...
        this.storeEntries();
        this.app.rpc.dispatch("fabric-i18n-entry-update", args);
    }
