...
class Application extends Module {
	constructor(options_ = { }) {
		...
		console.log(`µFabtic ${pkg.version}`);
		console.log(`µFabtic starting NATS...`);
		...
		if(options.nats){
			...
			if(options.nats.port){
				defaults.nats = Object.assign(defaults.nats, {
...
//			config = utils.getConfig(path.join(options.moduleFolder,options.configPath,options.ident), defaults);
		...
		let BIN = options.bin || options.µFabricBin || options.µFabricBinaries;
		...
		if(this.config.nats) {
			...
			this.proc.nats = new utils.Process({ 
				...
				args : [ BIN.NATS || BIN.nats, '--port', this.config.nats.port ],
				options : { cwd : BIN },
			})
		}
		...
		if(this.config.stan) {
			...
			this.proc.stan = new utils.Process({ 
				...
				args : [ BIN.STAN || BIN.stan, '--port', this.config.stan.port, 
	...
	//			console.log("RUNNING PROXY SERVER")
		...
		this.init((callback) => {
			if(options.hasModuleManager === false)
				return callback();
