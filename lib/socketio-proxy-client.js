...
class SocketIOProxyClient {
    ...
    connect(){
        ...
        this.rpc.emitAsync('rpc-connecting');
        this.socket = io(this.options.origin+this.options.path, this.options.args || {});
        //console.log("this.options.origin+this.options.path", this.options.origin+this.options.path)
        this.socket.on('ready', () => {
            this.online = true;
        })
        this.socket.on('connect', () => {
            this.rpc.attach(this.uid, this.socket);
            this.rpc.emit('rpc-connect');
        })
        this.socket.on('connect_error', (err) => {
            console.log("connect_error", err)
            this.rpc.emit('rpc-connect-error', err);
        })
        this.socket.on('error', (err) => { 
            console.log("RPC error", (err ? err.toString() : arguments));
        	this.rpc.emit('rpc-error', err);
        })
        this.socket.on('offline', () => {
            this.rpc.emit('offline');
        })
        this.socket.on('disconnect', () => { 
            this.online = false;
   			this.rpc.emit('rpc-disconnect');
